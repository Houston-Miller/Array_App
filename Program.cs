...
if (location != null && location.results != null)
{
    ...
    if (weatherData != null)
    {
        for (int i = 0; i < weatherData!.daily.time.Count; i++)
        {
            ...
            if (highTemp < 60)
            {
                Console.WriteLine($"On {date}: {todayWeather} - High: {highTemp}°F with a {rainChance}% chance of precipitation - Bring a Coat!");
            }
            else if (highTemp > 90)
            {
                Console.WriteLine($"On {date}: {todayWeather} - High: {highTemp}°F with a {rainChance}% chance of precipitation - Bring a water!");
            }
            else
            {
                Console.WriteLine($"On {date}: {todayWeather} - High: {highTemp}°F with a {rainChance}% chance of precipitation");
            }
        }
    }
}
